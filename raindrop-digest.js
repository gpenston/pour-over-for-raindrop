...
// Helper sleep
default async function sleep(ms) {
...
// Generate recommendations via OpenAI with exponential backoff
async function getRecommendations(archiveItems, count = 3) {
  const sample = archiveItems
    .slice(0, 3)  // use top 3 for concise context
    .map(i => `- ${i.title}`)
    .join('\n');
  ...
  const callOpenAI = () => axios.post(
    'https://api.openai.com/v1/chat/completions',
    {
      model: 'gpt-3.5-turbo',
      messages: [
        { role: 'system', content: 'You recommend content based on user history.' },
  ...
  while (attempt < maxAttempts) {
    try {
      ...
      if (err.response?.status === 429 && attempt < maxAttempts - 1) {
        console.warn(`⚠️ Rate limited, retrying in ${delays[attempt]/1000}s...`);
  ...
  if (!resp) {
    console.warn('⚠️ Max retries reached; skipping recommendations.');
    return [];
  }
...
(async()=>{
  try{
    ...
    if(saved.length<=5){ console.log('<=5 items, skip digest.'); return; }
    ...
    if(ARCHIVE_ID){
      try{
        ...
        console.warn('⚠️ Recommendation failed, sending without recs:',e.message);
      }
    } else console.warn('⚠️ ARCHIVE_ID not set');
